...
import { Plus, Trash2, Save, Loader2, Banknote, QrCode } from "lucide-react";

type Bank = { name: string; number: string; holder: string };
type Config = { banks: Bank[]; qrisImageUrl: string; instructions: string };

const EMPTY: Config = { banks: [], qrisImageUrl: "", instructions: "" };
...
export default function PaymentSettingsPage() {
  ...
  const addBank = () =>
    setConfig(prev => ({ ...prev, banks: [...prev.banks, { name: "", number: "", holder: "" }] }));
  ...
  const removeBank = (i: number) =>
    setConfig(prev => ({ ...prev, banks: prev.banks.filter((_, idx) => idx !== i) }));
    ...
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-brand-gray-500 text-sm mt-1">
          Info rekening bank & QRIS yang ditampilkan ke pembeli saat memilih transfer manual
        </p>
      </div>
        ...
        {config.banks.map((bank, i) => (
          <div key={i} className="grid grid-cols-3 gap-3 p-4 border border-brand-gray-800 relative">
            ...
            <div>
              ...
              <input value={bank.holder}
                     onChange={e => updateBank(i, "holder", e.target.value)}
      ...
      <div className="bg-brand-gray-900 border border-brand-gray-700 p-6 space-y-4">
        ...
        <div>
          ...
          <p className="text-[10px] text-brand-gray-600 mt-1">
            Upload foto QRIS kamu ke Google Drive / Imgur dan paste link-nya di sini
          </p>
        </div>
        {config.qrisImageUrl && (
          <div className="flex justify-center p-4 bg-white inline-block">
